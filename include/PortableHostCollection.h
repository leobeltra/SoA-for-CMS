...
using Vec1D = alpaka::Vec<Dim, Idx>;
...
public:
  ...
  using Buffer = alpaka::Buf<DevHost, std::byte, Dim, Idx>;
  using ConstBuffer = alpaka::ViewConst<Buffer>;

  PortableHostCollection() = default;

  PortableHostCollection(int32_t elements, DevHost const& host)
      // allocate pageable host memory
      : buffer_{alpaka::allocBuf<std::byte, Idx>(host, Vec1D::all(Layout::computeDataSize(elements)))},
        layout_{alpaka::getPtrNative(*buffer_), elements},
        view_{layout_} {
    // Alpaka set to a default alignment of 128 bytes defining ALPAKA_DEFAULT_HOST_MEMORY_ALIGNMENT=128
    // assert(reinterpret_cast<uintptr_t>(alpaka::getPtrNative(*buffer_)) % Layout::alignment == 0);
  }
...
template <typename TQueue, typename = std::enable_if_t<alpaka::isQueue<TQueue>>>
PortableHostCollection(int32_t elements, TQueue const& queue)
    : buffer_{alpaka::allocBuf<std::byte, Idx>(alpaka::getDev(queue), Vec1D(Layout::computeDataSize(elements)))},
      layout_{alpaka::getPtrNative(*buffer_), elements},
      view_{layout_} {
  // assert(reinterpret_cast<uintptr_t>(alpaka::getPtrNative(*buffer_)) % Layout::alignment == 0);
}
  ...
  static void ROOTReadStreamer(PortableHostCollection* newObj, Layout& layout) {
    ...
    new (newObj) PortableHostCollection(layout.metadata().size(), DevHost{});
...
public:
  ...
  PortableHostMultiCollection(int32_t elements, DevHost const& host)
      // allocate pageable host memory
      : buffer_{alpaka::allocBuf<std::byte, Idx>(host, Vec1D::all(Layout<>::computeDataSize(elements)))},
        impl_{alpaka::getPtrNative(*buffer_), elements} {
    // Alpaka set to a default alignment of 128 bytes defining ALPAKA_DEFAULT_HOST_MEMORY_ALIGNMENT=128
    // assert(reinterpret_cast<uintptr_t>(alpaka::getPtrNative(*buffer_)) % Layout<>::alignment == 0);
  ...
  PortableHostMultiCollection(const std::array<int32_t, members_>& sizes, DevHost const& host)
      // allocate pinned host memory associated to the given work queue, accessible by the queue's device
      : buffer_{alpaka::allocBuf<std::byte, Idx>(host, Vec1D::all(computeDataSize(sizes)))},
        impl_{alpaka::getPtrNative(*buffer_), sizes} {
    // Alpaka set to a default alignment of 128 bytes defining ALPAKA_DEFAULT_HOST_MEMORY_ALIGNMENT=128
    portablecollection::constexpr_for<0, members_>(
        [&](auto i) { assert(reinterpret_cast<uintptr_t>(alpaka::getPtrNative(*buffer_)) % Layout<i>::alignment == 0); });
  ...
  PortableHostMultiCollection(const std::array<int32_t, members_>& sizes, TQueue const& queue)
      // allocate pinned host memory associated to the given work queue, accessible by the queue's device
      : buffer_{alpaka::allocBuf<std::byte, Idx>(alpaka::getDev(queue), Vec1D(computeDataSize(sizes)))},
        impl_{alpaka::getPtrNative(*buffer_), sizes} {
    // Alpaka set to a default alignment of 128 bytes defining ALPAKA_DEFAULT_HOST_MEMORY_ALIGNMENT=128
    portablecollection::constexpr_for<0, members_>(
        [&](auto i) { assert(reinterpret_cast<uintptr_t>(alpaka::getPtrNative(*buffer_)) % Layout<i>::alignment == 0); });
  ...
  static void ROOTReadStreamer(PortableHostMultiCollection* newObj, Implementation& onfileImpl) {
    ...
    new (newObj) PortableHostMultiCollection(sizes, DevHost{});
