...
class PortableDeviceCollection {
  static_assert(not std::is_same_v<TDev, alpaka::DevCpu>,
...
public:
  ...
  using Buffer = alpaka::Buf<TDev, std::byte, Dim, Idx>;
  ...
  PortableDeviceCollection(int32_t elements, TDev const& device)
      : buffer_{alpaka::allocBuf<std::byte, Idx>(device, Vec1D::all(Layout::computeDataSize(elements)))},
...
public:
  ...
  PortableDeviceMultiCollection(const SizesArray& sizes, TDev const& device)
      ...
      : buffer_{alpaka::allocBuf<std::byte, Idx>(device, Vec1D::all(computeDataSize(sizes)))},
